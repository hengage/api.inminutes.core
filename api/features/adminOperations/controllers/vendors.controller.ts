...
export class AdminOpsVendorsController {
  ...
  getVendor = async (req: Request, res: Response): Promise<void> => {
    try {
      const vendor = await this.adminVendorsService.getVendor(
        req.params.vendorId
      );
  ...
  updateVendor = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const vendor = await this.adminVendorsService.updateVendor(
        req.params.vendorId,
        req.body
      );
      handleSuccessResponse(
  ...
  deleteVendor = async (req: Request, res: Response): Promise<void> => {
    try {
      await this.adminVendorsService.deleteVendor(req.params.vendorId);
      handleSuccessResponse(
        res,
        HTTP_STATUS_CODES.NO_CONTENT,
        null,
        `Deleted vendor with ID ${req.params.vendorId}`
      );
    } catch (error: unknown) {
      const { statusCode, errorJSON } = handleErrorResponse(error);
      res.status(statusCode).json(errorJSON);
